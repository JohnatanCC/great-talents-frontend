...
const levelToPct = (lvl?: string) => {
    const v = (lvl || "").toLowerCase();
    if (/^basi|^inic/.test(v)) return 30;
    if (/^inter/.test(v)) return 60;
    if (/^avan|^upper|^flu/.test(v)) return 85;
    if (/^expe|^nati|^nativo/.test(v)) return 100;
    return 50;
};
...
const SkillMeter = ({ name, level }: { name: string; level?: string }) => (
    <Box>
        <HStack justify="space-between" mb={1}>
            <Text fontWeight={600} fontSize="sm">{name}</Text>
            <Text fontSize="xs" color="muted">{level || "—"}</Text>
        </HStack>
        <Progress value={levelToPct(level)} size="sm" colorScheme="brand" borderRadius="sm" />
    </Box>
);
